...
std::string RandomString(const size_t length) {
	std::string r;
	static const char bet[] = { "ABCDEFGHIJKLMNOPQRSTUVWXYabcdefghijklmnopqrstuvwxyz1234567890" };
...
int main() {
	...
	if (Functions::DoesFileExist("NameOfCheat.dll")) {
		if (!Functions::Internal::ExecuteBypass(game)) {
			Cleanup("Cannot bypass.");
		}

		if (Functions::LoadLibraryInject(ProcessId, "NameOfCheat.dll")) {
